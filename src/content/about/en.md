---
bio:
  zh: |
    我是李俊森，南京大学工程管理学院计算机金融实验班 2025 级本科生（大二在读，GPA 4.28/5.0），聚焦量化研究与金融科技方向。

    我坚持"金融逻辑为核心、代码工具为辅"的复合路线：一方面系统修读公司金融、金融经济学、货币银行学等核心课程，另一方面能够独立完成从数据清洗、因子构建到回测验证的量化研究闭环。

    2026 年暑期，我在星环科技（Transwarp）人工智能产品部担任量化研究员，作为 5 人投资决策委员会成员（持风控否决权）参与 AI-Native 组合投资项目：主导开发多因子选股回测系统 AlphaForge 与事件驱动回测引擎，主导设计"工程 / 数据 / 认知"三大飞轮自进化系统，并作为第三发明人参与一项面向大语言模型智能体可信控制的国家发明专利申请（申请中）。

    我习惯"结论必须可追溯"——每条规则有依据、每个结果可复盘，期望在量化研究与金融科技方向长期发展。欢迎通过邮件与我交流。
  en: |
    I am Junsen Li, a sophomore in the CS-Finance Experimental Class at the School of Engineering Management, Nanjing University (GPA 4.28/5.0), focusing on quantitative research and fintech.

    I follow an interdisciplinary path — finance logic at the core, code as the tool: rigorous coursework in Corporate Finance, Financial Economics, and Money & Banking, combined with the ability to independently run the full quantitative research loop, from data cleaning and factor construction to backtesting and validation.

    In summer 2026, I interned as a quantitative researcher at Transwarp Technology's AI Product Department. As a member of a five-person investment committee with risk veto in an AI-native portfolio program, I led the development of AlphaForge (a multi-factor screening and backtesting system) and an event-driven backtesting engine, led the design of a three-flywheel self-evolution system, and contributed as third inventor to a pending national patent on trustworthy control of LLM-based agents.

    I believe every conclusion must be traceable — every rule has a rationale, and every result can be audited. Feel free to reach out by email.
education:
  - degree:
      zh: "本科 · 计算机金融实验班"
      en: "B.Eng. Candidate, CS-Finance Experimental Class"
    institution:
      zh: "南京大学 · 工程管理学院"
      en: "School of Engineering Management, Nanjing University"
    year: "2025.09 – 2029.06（预计）"
    description:
      zh: "GPA 4.28 / 5.0；计算机技术 × 金融学复合培养；核心课程：公司金融学、金融经济学、货币银行学、概率论与数理统计、Python 数据挖掘；英语 CET-4 579（CET-6 备考中）"
      en: "GPA 4.28 / 5.0; interdisciplinary CS × Finance program. Core courses: Corporate Finance, Financial Economics, Money & Banking, Probability & Statistics, Python Data Mining. English: CET-4 579 (CET-6 in progress)"
experience:
  - title:
      zh: "量化研究员（实习）"
      en: "Quantitative Researcher (Intern)"
    institution:
      zh: "星环科技 Transwarp · 研发中心 / 人工智能产品部"
      en: "Transwarp Technology · R&D Center / AI Product Dept."
    period: "2026.07 – 2026.08"
    description:
      zh: "AI-Native 组合投资项目 · 5 人投资决策委员会成员（持风控否决权），负责因子打分、组合构建、回测校验、风险检查与归因复盘"
      en: "AI-native portfolio program · member of a 5-person investment committee with risk veto; owned factor scoring, portfolio construction, backtest validation, risk checks, and attribution review"
awards:
  - title:
      zh: "发明专利《一种智能体不确定性可信约束的方法与系统》（申请中）"
      en: "Invention Patent (pending): Method and System for Trustworthy Uncertainty Constraint of AI Agents"
    year: 2026
    description:
      zh: "第三发明人；面向大语言模型智能体的可信控制技术，约束多步骤执行中的行为、语义与溯源不确定性"
      en: "3rd inventor; trustworthy-control technology for LLM-based agents, constraining behavioral, semantic, and provenance uncertainty in multi-step execution"
  - title:
      zh: "学业表现 GPA 4.28 / 5.0"
      en: "Academic Standing: GPA 4.28 / 5.0"
    year: 2026
    description:
      zh: "南京大学 · 计算机金融实验班"
      en: "CS-Finance Experimental Class, Nanjing University"
---

Undergraduate in the CS-Finance Experimental Class at Nanjing University, focusing on quantitative research and fintech: multi-factor stock screening, event-driven backtesting, AI agents, and data analytics. Former quantitative researcher intern at Transwarp Technology's AI Product Department, working on an AI-native portfolio program; led development of AlphaForge and a three-flywheel self-evolution system. Programmer for the China Undergraduate Mathematical Contest in Modeling (CUMCM, in preparation) and core developer of the MEM course project "Data Analytics Assistant for Enterprise Management".
